...
class FileHandler:
        ...
        with open(filename) as f:
            if read_mode == 'char':  # Read file as characters
                ...
                for message in lines:
                    contents.extend([*message])  # Extend the list with individual characters
    ...
    def write(self, filename: str, content: str):
        ...
        if not filename.lower().endswith(('.txt')):  # Check if the file type is valid
            ...
            while handle_file_exists not in {'O', 'A', 'F'}:  # Prompt user for action until a valid choice is made
            ...
            if handle_file_exists == 'O':  # Overwrite existing file
                ...
                with open(filename, 'a') as f:
                    content = '='*60 + '\n' + content  # Add separator line before appending
                    f.write(content)
            elif handle_file_exists == 'F':  # Write to a different file
                new_file = input('New file name: ')
                self.write(new_file, content)  # Call write method recursively with a new file name
    ...
    def read_folder(self, folder: str):
        ...
        with os.scandir(f'{folder}/') as dir_files:  # Iterate over files in the directory
            for file in dir_files:
                file = File(file.name)  # Create a File object for each file in the directory
